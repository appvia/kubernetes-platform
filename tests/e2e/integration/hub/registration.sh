...
@test "We should have a cluster registration application pointing at the correct git repository" {
  kubectl_argocd "get application system-registration-hub -o yaml | yq .spec.sources[0].repoURL | grep -i https://github.com/appvia/kubernetes-platform"
  kubectl_argocd "get application system-registration-spoke -o yaml | yq .spec.sources[0].repoURL | grep -i https://github.com/appvia/kubernetes-platform"
}
...
@test "We should have a cluster secret with a platform_repository annotation" {
  kubectl_argocd "get secret cluster-hub -o yaml | yq .metadata.annotations.platform_repository | grep -i https://github.com/appvia/kubernetes-platform"
}
...
@test "We should have a cluster secret with a tenant repository annotation" {
  kubectl_argocd "get secret cluster-hub -o yaml | yq .metadata.annotations.tenant_repository | grep -i https://github.com/appvia/kubernetes-platform"
}
